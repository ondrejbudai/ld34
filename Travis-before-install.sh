...
case "$os" in
  linux32)
    ...
    sudo i386 chroot "$chroot" apt-get install -qq libsdl2-dev libsdl2-image-dev g++-4.8 build-essential zip
  ...
  linux64)
    ...
    sudo apt-get update -qq
    sudo apt-get install -qq libsdl2-dev libsdl2-image-dev g++-4.8
    ;;
  win32)
